...
const noMoreQuestions = computed(() => {
  ...
  // Użyj bezpośrednio danych z cache zamiast wywoływać asynchroniczną funkcję
  ...
  try {
    ...
    if (route.query.ids) {
      ...
      if (filteredQuestions.length > 0) {
        questions.value = getRandomUniqueQuestions(filteredQuestions, quizLength.value);
        ...
        console.warn('No questions matched the specified IDs');
      }
    }

    // Jeśli nie mamy konkretnych IDs lub filtracja nie zwróciła wyników, użyj wszystkich pytań
...
const selectAnswer = async (index, selectedKey) => {
  ...
  try {
    ...
    // Dodaj aktualizację cache po zapisaniu odpowiedzi
...
const saveUserHistory = async () => {
  try {
    ...
    // WAŻNA ZMIANA: Zaktualizuj lokalny stan hquestion natychmiast
    const updatedHQ = [...(store.state.user.hquestion || [])];

    // Dodaj wpisy o odpowiedziach do lokalnego stanu
    questions.value.forEach((q, idx) => {
      const questionId = q.ID || q.id || q.Id || q.id_question;
      const isCorrect = answersStatus.value[idx].selectedKey === getCorrectKey(q);

      // Sprawdź, czy wpis już istnieje i zaktualizuj lub dodaj nowy
      const existingIndex = updatedHQ.findIndex(
        (item) => String(item.id) === String(questionId) && item.category === q.category
      );
    ...
    // Zaktualizuj store bezpośrednio
    store.commit('user/SET_HQUESTION', updatedHQ);

    // Teraz zaktualizuj cache
    await updateWrongOrNotDoneCache();

    // Wymuszamy aktualizację komponentu
    nextTick(() => {
      // Opcjonalne: odśwież UI
    });
...
const restartQuiz = async (onlyFailedOrNotAnswered = false) => {
  ...
  if (onlyFailedOrNotAnswered && route.query.categories) {
    const cat = route.query.categories;
    try {
      // Użyj zaktualizowanego cache
      const wrongOrNotDoneIds = wrongOrNotDoneIdsCache.value;

      if (wrongOrNotDoneIds.length === 0) {
        // Jeśli nie ma już pytań do powtórki, wróć do strony głównej
        router.push('/');
      ...
      router.replace({
        ...
        query: {
          ...
          length,
          onlyNotDone: 'true', // Dodane, aby upewnić się że parametr jest przekazany
...
const retryWrongQuestions = () => {
  const wrongIndexes = answersStatus.value
    .map((a, idx) => (a.selectedKey !== getCorrectKey(questions.value[idx]) ? idx : null))
    .filter((idx) => idx !== null);
...
// Implementacja funkcji categoryStats
const categoryStats = async (cat) => {
  try {
    // Pobierz historię pytań użytkownika
    const historyRes = await apiClient.get('/users/hquestion');
    const hquestion = historyRes.data;

    // Pobierz wszystkie pytania z kategorii
    const allQuestions = await store.dispatch('questions/fetchQuestionsByCategory', cat);
    const allIds = allQuestions.map((q) => q.ID || q.id || q.Id || q.id_question);

    // Zlicz pytania odpowiedziane poprawnie i błędnie
    const answeredQuestions = hquestion.filter((q) => q.category === cat);
    const correct = answeredQuestions.filter((q) => q.correct === true).length;
    const wrong = answeredQuestions.filter((q) => q.correct === false).length;

    // Oblicz liczbę nieprzerobionych pytań
    const answeredIds = answeredQuestions.map(
      (q) => String(q.id) || String(q.ID) || String(q.question_id)
    );

    const notDone = allIds.filter((id) => !answeredIds.includes(String(id))).length;

    return { correct, wrong, notDone };
  } catch (error) {
    console.error('Błąd podczas pobierania statystyk kategorii:', error);
    return { correct: 0, wrong: 0, notDone: 0 };
  }
};

// Obserwuj zmiany w answersStatus i aktualizuj cache, gdy wszystkie pytania zostały odpowiedziane
watch(
  () => answersStatus.value.filter((a) => a.answered).length,
  async (newCount) => {
    if (newCount === questions.value.length && !showSummary.value) {
      // Gdy wszystkie pytania zostały odpowiedziane, a jeszcze nie pokazujemy podsumowania
      await updateWrongOrNotDoneCache();
    }
  }
);

// Dodaj tę funkcję w QuizView.vue
const forceDataRefresh = async () => {
  try {
    // Wymuszamy pełne odświeżenie danych z serwera
...
const getQuestionIdsForRetry = () => {
  // Implementacja zależy od struktury danych, ale ogólnie:
  // - Pobierz wszystkie dostępne pytania z kategorii
  // - Pobierz historię użytkownika dla danej kategorii
  // - Zwróć ID pytań, których nie ma w historii lub odpowiedź była błędna
  const allIds = questions.map((q) => q.id || q.ID || q.Id || q.id_question);
  const hquestion = store.getters['user/getHquestion'] || [];
  const hq = hquestion.filter((q) => q.category === category);
...
const startQuizWithIds = (ids) => {
  // Kod do uruchomienia quizu z określonymi ID pytań
  // Przykładowa implementacja:
  router.push({
    name: quizType === 'exam' ? 'ExamView' : 'QuizView',
    query: {
      length: ids.length,
      categories: category,
      onlyNotDone: true,
