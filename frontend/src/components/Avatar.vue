<template>
  <div class="avatar" :class="sizeClass">
    <svg viewBox="0 0 24 24" :width="size" :height="size" xmlns="http://www.w3.org/2000/svg">
      <g :fill="safeColor">
        <!-- Wszystkie templaty avatar pozostają bez zmian -->
...
export default {
  ...
  props: {
    ...
    color: {
      type: String,
      default: '#3B82F6', // Domyślny kolor niebieski
      validator: (value) => {
        // Upewnij się, że wartość jest stringiem zawierającym poprawny kolor
        if (typeof value === 'number') {
          console.warn(
            'Avatar: prop "color" otrzymał liczbę zamiast stringa. Konwersja automatyczna.'
          );
        }
        return true; // Konwersja nastąpi w computed property
      },
    },
    size: {
      type: Number,
  ...
  computed: {
    avatarType() {
      return Math.abs(this.avatarIndex % 17);
    },
    safeColor() {
      // Dodajemy computed property, która zawsze zwraca poprawny kolor
      if (typeof this.color === 'string') {
        return this.color;
      }
      // Jeśli to nie jest string, zwracamy domyślny niebieski
      return '#3B82F6';
    },
    sizeClass() {
      // Klasy bazowane na wielkości awatara
      if (this.size <= 24) return 'avatar-sm';
      if (this.size >= 48) return 'avatar-lg';
      return 'avatar-md';
    },
  },
};
...
.avatar {
  ...
  border-radius: 50%;
  overflow: hidden;
  /* Dodaj wymiary zgodne z props size */
  width: v-bind(size + 'px');
  height: v-bind(size + 'px');
}

/* Przywróć normalne pozycjonowanie SVG */
.avatar svg {
  width: 100%;
  height: 100%;
  /* Usuń absolute positioning, które powodowało problem */
  position: relative;
  transform: none;
  top: auto;
  left: auto;
